...
BootstrapGame({
    configPath: ["gameforge.config.json", "Demos/DemoHello/hello.config.json"],
    manifestPath: "Demos/DemoHello/resources.json",
...
`Demos/DemoHello2D/hello2d.config.json`:
...
`Demos/DemoHello2D/Assets/Manifests/room.json`:
...
`Demos/DemoHello2D/resources.json`:

```json
{
  "jsons": [
    { "name": "hello_room", "path": "Demos/DemoHello2D/Assets/Manifests/room.json" }
  ]
}
```
...
`Demos/DemoHello2D/Player.js`:

```js
import { ActionManager, Vector2D } from "../../CoreCross/index.js";
import { Draw, GameObject } from "../../Core2D/index.js";
...
`Demos/DemoHello2D/Hello2DLevel.js`:

```js
import { AssetManager, Level, Vector2D } from "../../CoreCross/index.js";
import { Collide2D, Draw, GameObject, Screen } from "../../Core2D/index.js";
...
`Demos/DemoHello2D/mainHello2D.js`:
...
BootstrapGame({
    configPath: ["gameforge.config.json", "Demos/DemoHello2D/hello2d.config.json"],
    manifestPath: "Demos/DemoHello2D/resources.json",
    levels: [new Hello2DLevel()],
});
```

Register it in `CoreCross/DemoLauncher.js`, then open
...
`Demos/DemoHello3D/resources.json`:

```json
{
  "jsons": [
    { "name": "hello_scene", "path": "Demos/DemoHello3D/Assets/Manifests/first.scene.json" }
  ]
}
```

`Demos/DemoHello3D/hello3d.config.json`:
...
`Demos/DemoHello3D/Hello3DLevel.js`:
...
`Demos/DemoHello3D/mainHello3D.js`:
...
BootstrapGame({
    configPath: ["gameforge.config.json", "Demos/DemoHello3D/hello3d.config.json"],
    manifestPath: "Demos/DemoHello3D/resources.json",
...
```js
import { Vector3D } from "../../CoreCross/index.js";
...
```js
import { Camera3D, LegacyLevel3D, Mesh3D, Shapes3D, Skybox3D } from "../../Core3D/index.js";
```
...
- `Demos/Demo3D` for a small renderer showcase.
- `Demos/DemoSolarSystem` for custom materials and many animated bodies.
- `Demos/DemoMiniGame3D` for manifests, physics, gamepad, skybox and PBR surfaces.
