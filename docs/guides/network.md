...
O chat visual da demo usa `src/Core2D/UI/ChatWindow.js`, composto com o `TextBox` da engine e desenhado no canvas. Nenhum painel HTML externo e criado.
...
O mapa, blocos, aparencia e regras ficam em `examples/DemoOnlineMMO`, incluindo `data/OnlineWorldData.js`. Eventos de bloco sao mensagens que a propria demo escolhe interpretar.
...
```js
import { GameNetwork, WebSocketClientAdapter } from "../src/CoreNetwork/index.js";
