...
`+ Scene` and `Save`. `examples/Demo3D` is a ready-to-open example whose runtime
...
{
  "jsons": [
    { "name": "scene", "path": "examples/My3DGame/Assets/Manifests/scenes/start.scene.json" }
  ]
}
```

`examples/DemoMiniGame3D` is also ready to open as a project. Its workspace maps
