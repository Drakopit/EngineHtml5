...
function run() {
    ...
    if (config.engineModules && config.engineModules.length > 0) {
        for (const mod of config.engineModules) {
            ...
            // But a simple approach is just to mirror the configured path in the output
            // so the game can reference copied engine modules with the same relative layout.
